...
class Inv_keluar extends CI_Controller
{
    ...
    public function detail($id)
    {
        ...
        // Logged in
            ...
            if ($id != "") {
                ...
                $this->data['data_detail'] = $this->inv_keluar_model->get_inventory_by_id(
                    $id
                );
        ...
        // Logged in
            ...
            if (isset($_POST) && !empty($_POST)) {
                ...
                if ($this->form_validation->run() === TRUE) {
                    ...
                    if ($this->inv_keluar_model->update_inventory_by_id($id, $data)) {
                        // Set message
                        $this->session->set_flashdata(
                            'message',
            ...
            // Get data
            $this->data['id']       = $id;
            $this->data['data_list']  = $this->inv_keluar_model->get_inventory_by_id($id);
            $this->data['cat_list']   = $this->categories_model->get_categories('', '', '', 'asc');
            $this->data['stat_list']  = $this->status_model->get_status('', '', '', 'asc');
            $this->data['loc_list']   = $this->locations_model->get_locations('', '', '', 'asc');
            $this->data['col_list']   = $this->color_model->get_color('', '', '', 'asc');
    ...
    public function add()
    {
        ...
        // Jika login
            ...
            // extract flashdata
            $agent = $this->session->flashdata('agent');
            ...
            if ($this->inv_keluar_model->insert_data($agent, $code, $status, $jumlah_datas, $description)) {
                $this->session->set_flashdata(
                    'message',
                    $this->config->item('success_start_delimiter', 'ion_auth')
                        . "Inventory Moved !" .
        ...
        // Logged in
            ...
            if (isset($_POST) && !empty($_POST)) {
                ...
                if ($this->form_validation->run() === TRUE) {
                    ...
                    $this->data['data_list']  = $this->inv_keluar_model->get_inventory('','','',$date);
