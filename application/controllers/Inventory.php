...
class Inventory extends CI_Controller
{
	...
	public function add()
	{
		...
		// Logged in
			...
			$this->form_validation->set_rules('agent', 'agent', 'trim|required|addslashes');
			...
			$this->form_validation->set_rules('satuan_datas', 'satuan_datas', 'trim|addslashes');
            ...
            $this->form_validation->set_rules('time_of_purchase', 'Date of Purchase', 'trim|required');
			...
			if (isset($_POST) && !empty($_POST)) {
				...
				if ($this->form_validation->run() === TRUE) {
					...
					$data = array(
						...
						'color'            => $color,
						'jumlah_awal'      => $this->input->post('jumlah_datas'),
						'jumlah_datas'     => $this->input->post('jumlah_datas'),
						'satuan_datas'	   => $this->input->post('satuan_datas'),
						...
						'time_of_purchase' => $this->input->post('time_of_purchase'),
	...
	public function move()
	{
		...
		// Logged in
			...
			$this->form_validation->set_rules('jumlah_datas', 'jumlah_datas', 'numeric|trim');
			$this->form_validation->set_rules('date_of_sale', 'Date of Sale', 'trim|required');
            $this->form_validation->set_rules('time_of_sale', 'Date of Sale', 'trim|required');
			...
			if (isset($_POST) && !empty($_POST)) {
				...
				if ($this->form_validation->run() === TRUE) {
					...
					$time_of_sale=$this->input->post('time_of_sale');
	...
	public function print()
	{
		...
		// Logged in
			...
			$this->form_validation->set_rules('tanggal_cetak', 'tanggal_cetak', 'trim|addslashes');
			// klik submit
			if (isset($_POST) && !empty($_POST)) {
				// validation run
				if ($this->form_validation->run() === TRUE) {
					$date = $this->input->post('tanggal_cetak');
					$this->data['data_list']  = $this->inventory_model->get_inventory('', '', '', $date);
	...
	public function resorting()
	{
		...
		// Logged in
			...
			$this->form_validation->set_rules('code', 'code', 'trim|addslashes');
			// klik submit
			if (isset($_POST) && !empty($_POST) && $this->form_validation->run() === TRUE) {
				$code=$this->input->post('code');
				$list[] = $this->inventory_model->select_code_array($code);
				// CODE A0044
				// echo "<pre>";
				// 	print_r($list);
				// echo "</pre>"; 
				...
				if ($list[0]!=''){
					for ($i=0; $i<$j; $i++){
						$array_code = explode("-",$list[0][$i]['code']);
						$strlen    = strlen($i);
						if ($strlen == 3) {
							$last_code = "0" . ($i+1);
						} elseif ($strlen == 2) {
						...
						$new_code=$array_code[0]."-".$array_code[1]."-".$last_code;
						$id=$list[0][$i]['id'];
						$datas=array('code'=>$new_code); 
						if($this->inventory_model->update_inventory_by_id($id,$datas)){
							$this->session->set_userdata(
								'message',
								$this->config->item('message_start_delimiter', 'ion_auth')
									. "Inventory Update Failed!" .
