...
class Inventory_model extends CI_Model
{
	...
	public function get_inventory($id = '', $limit = '', $start = '', $created_on = '')
	{
		...
		if ($created_on != '') {
			$this->db->like($this->datas_table . '.date_of_purchase', $created_on);
		}
		...
		// order by
		// $this->db->order_by($this->datas_table . '.created_on', 'asc');
		$this->db->order_by($this->datas_table . '.date_of_purchase', 'asc');
		$this->db->order_by($this->datas_table . '.time_of_purchase', 'asc');
		$datas = $this->db->get();
		return $datas;
	}

	public function select_code_array($code=""){
		$this->db->select(
			$this->datas_table . ".id, " .
			$this->datas_table . ".code"
		);
		...
		// if provided
		if ($code!=''){
			$this->db->like($this->datas_table . '.code', $code);
		}
		// order by
		$this->db->order_by($this->datas_table . '.created_on', 'asc');
